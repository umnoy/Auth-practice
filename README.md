...
4. Теперь можно добавлять новых пользователей, ввод данных там тоже защищен:
    1. Валидация username — ^[a-zA-Z0-9_]{3,30}$}, только буквы, цифры, _.
    2. Jinja2 auto-escaping, все {{ }} в шаблонах автоматически экранируют HTML, защита от XSS.
    3. HTML5-валидация на клиенте pattern, maxlength, minlength, required как дополнительный барьер.
    4. Проверка длины пароля (минимум 6 символов) и совпадения паролей на сервере.
